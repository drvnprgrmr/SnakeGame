...
void nvs_read_str(nvs_handle_t *handle, char *k, char **v)
{
    ...
    if (err == ESP_OK)
    {
        ...
        *v = malloc(required_size);
        ...
        if (err == ESP_OK)
        {
            ESP_LOGI(TAG, "Read string: %s, size: %i", *v, required_size);
        }
        else
        {
            ESP_LOGI(TAG, "Couldn't read string. %s", esp_err_to_name(err));
        }
    }
    else
    {
        ESP_LOGI(TAG, "Couldn't get size of string. %s", esp_err_to_name(err));
    }
}